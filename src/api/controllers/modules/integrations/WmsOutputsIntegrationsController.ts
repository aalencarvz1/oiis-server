    ...
    static async get_loads_with_broker(req: Request, res: Response, next: NextFunction) : Promise<void> {
        try {
            ...
            if (Utils.typeOf(id) !== 'array') {
                ...
                id = id.map((el: any)=>Utils.hasValue(el) ? el : 'null');
            }
            let codFilial = params?.codFilial || [];
            if (Utils.typeOf(codFilial) !== 'array') {
                codFilial = codFilial.toString().split(',');
                codFilial = codFilial.map((el: any)=>Utils.hasValue(el) ? el : 'null');
            }
            ...
            if (Utils.typeOf(winthorLoadNumber) !== 'array') {
                ...
                winthorLoadNumber = winthorLoadNumber.map((el: any)=>Utils.hasValue(el) ? el : 'null');
            }
            ...
            if (Utils.typeOf(auroraLoadNumber) !== 'array') {
                ...
                auroraLoadNumber = auroraLoadNumber.map((el: any)=>Utils.hasValue(el) ? el : 'null');
            }
            ...
            let query = `
                ...
                group by 
                    ...
                    u.id 
            `
            res.data = await DBConnectionManager.getConsultDBConnection()?.query(query,{raw:true,type: QueryTypes.SELECT});
            ...
            if (identifiers.length > 1) {
                ...
                let query = `
                    ...
                    from
                        EP.EPUNIFCARGAS u
                    where                        
                        ${identifiers.map((el: any)=>`(u.IDORIGEMINFO = ${el.idorigin} AND u.NRCARGA = ${el.cargo_number})`).join(' or ')}
                `
                let data : any = await DBConnectionManager.getConsultDBConnection()?.query(query,{raw:true,type:QueryTypes.SELECT});
                if (data.length) {
                ...
                if (Utils.toBool(await Parameter_Values.get(Parameters.HAS_WINTHOR_INTEGRATION)) === true && Utils.toBool(await Parameter_Values.get(Parameters.WMS_OUTPUT_INTEGRATION_CHECK_RCA)) === true) {
                    ...
                    if (loadingsWinthor.length) {
                        ...
                        `;
                        rcasWinthor = await DBConnectionManager.getConsultDBConnection()?.query(query,{raw:true,type:QueryTypes.SELECT});
                    }
                    ...
                    if (loadingsBroker.length) {
                        ...
                        `;
                        rcasBroker = await DBConnectionManager.getConsultDBConnection()?.query(query,{raw:true,type:QueryTypes.SELECT});
                    }
                ...
                data = await DBConnectionManager.getConsultDBConnection()?.query(query,{raw:true,type:QueryTypes.SELECT});
                let proxId = data[0].PROXID || 1;
                res.data = proxId;
                let queries = identifiers.map((el: any)=>`insert into EP.EPUNIFCARGAS values (${proxId},${el.idorigin},${el.cargo_number},sysdate,${req.user.id})`);
                for (let key in queries) {
                    await DBConnectionManager.getConsultDBConnection()?.query(queries[key],{type:QueryTypes.INSERT});
                }
                res.sendResponse(200,true);
    ...
    static async get_loadings_items_for_print(req: Request, res: Response, next: NextFunction) : Promise<void> {
        try {
            ...
            if (identifiers.length > 1) {
                query = `
                    ...
                    from
                        EP.EPUNIFCARGAS
                    where
                        ${identifiers.map((el: any)=>` (IDORIGEMINFO=${el.idorigin} AND NRCARGA=${el.cargo_number}) `).join(' OR ')}
                `
                data = await DBConnectionManager.getConsultDBConnection()?.query(query,{raw:true,type:QueryTypes.SELECT});
                if (data.length) {
            ...
            if (identifiers.length) {
                ...
                if (Utils.typeOf(positionOrders) !== 'array') positionOrders = positionOrders.toString().split(',');
