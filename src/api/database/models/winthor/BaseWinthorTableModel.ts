...
export default class BaseWinthorTableModel extends BaseTableModel {    
    ...
    static async runUpMigration(queryInterface: QueryInterface, options?: any) {
        ...
        await queryInterface.bulkInsert('tables',[{      
            ...
            created_at: new Date(),
            is_sys_rec : 1,
            data_connection_id : (config as any)[`${process.env.NODE_ENV||'development'}_winthor`].id,
            schema_id : (config as any)[`${process.env.NODE_ENV||'development'}_winthor`].id,
            name : this.tableName
        }],{
     ...
     * @created 2023-11-10
     */
    static async initModel(pSequelize?: any) : Promise<any> {
        try {
