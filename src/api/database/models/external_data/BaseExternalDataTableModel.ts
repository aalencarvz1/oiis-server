...
export default class BaseExternalDataTableModel extends BaseTableModel {    
    ...
    static async runUpMigration(queryInterface: QueryInterface, options?: any) {
        ...
        await queryInterface.bulkInsert('tables',[{      
            ...
            created_at: new Date(),
            is_sys_rec : 1,
            data_connection_id : (config as any)[`${process.env.NODE_ENV||'development'}_external_data`].id,
            schema_id : (config as any)[`${process.env.NODE_ENV||'development'}_external_data`].id,
            name : this.tableName
        }],{
