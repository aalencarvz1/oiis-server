...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    CREATE  TRIGGER TRG_TIMETASKTRACKER BEFORE UPDATE ON tasks_x_status_x_users FOR EACH ROW BEGIN
          ...
          CASE
            WHEN COALESCE((SELECT COUNT(1) FROM user_profile_timeworks UP JOIN user_timeworks ut ON ut.iduserprofiletimework = up.id WHERE UP.IDUSER = new.IDUSER and up.name = 'DEFAULT'),0) > 0 THEN
              ...
              TIMESTAMPDIFF(
                ...
                coalesce(
                new.LASTRUN,
                (SELECT MAX(T2.created_at) FROM tasks_x_status_x_users_logs T2 WHERE T2.IDTASKXSTATUSXUSER = new.id AND T2.IDNEWSTATUS=2),
                utc_timestamp
                ),
                utc_timestamp
              )
