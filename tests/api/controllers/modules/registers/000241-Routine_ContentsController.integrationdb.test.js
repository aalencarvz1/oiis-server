import Routine_ContentsController from "../../../../../dist/api/controllers/modules/registers/Routine_ContentsController";
import Routines_ContentsController from "../../../../../dist/api/controllers/modules/registers/Routine_ContentsController";
...
describe(Routines_ContentsController.name, () => {
    ...
    test('table class model name', () => {
        let tableClassModel = Routines_ContentsController.getTableClassModel();
     ...
     test('put without data', async () => {
            let result = await Routines_ContentsController._put({}) 
    ...
    //test put (insert)
    test('put', async () => {
        let resultRoutine = await HelperTestController.RoutinesCompletInsert(stringTest);

        let result = await Routines_ContentsController._put({
            routine_id:resultRoutine[0].id,
            name: stringTest,
        });
        expect(Utils.hasValue(result)).toBeTruthy();
        ...
        result = await Routines_ContentsController._get({
            where:{
                name: stringTest,            
            }
        });
     ...
     test('put Duplicate', async () => {
        let resultRoutine = await HelperTestController.RoutineGet(stringTest);

        let result = await Routine_ContentsController._put({
            routine_id:resultRoutine[0].id,
            name:stringTest,
        })
        expect(Utils.hasValue(result)).toBeTruthy();
    ...
    test('get', async () => {
        let result = await Routines_ContentsController._get({
            where:{
                name: stringTest,    
            }
        });
    ...
    test('patch', async () => {
        ...
        //update register
        result = await Routines_ContentsController._patch({
            where: {
                id: id
            },
            values:{
    ...
    test('delete', async () => {
        ...
        //update register
        result = await Routines_ContentsController._delete({
            where: {
                id: id
            }
        });
